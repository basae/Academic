...
if((isset($_SESSION['user']))&&($_SESSION['user']=="master") ){
	...
	$consulta=mysql_query("insert into socio(rfc,nombre,ap,am,direccion,telefono,email) values('$rfc','$nombre','$ap','$am','$direccion','$telefono','$email')",$con)or die(
	...
	echo "<script>alert('Se ha Guardado Correctamente');location.href='versocios.php' </script>";
		...
		mysql_query("update socio set rfc='$rfc',nombre='$nombre',ap='$ap',am='$am',direccion='$direccion',telefono='$telefono',email='$email' where id_socio=$id")or die(
